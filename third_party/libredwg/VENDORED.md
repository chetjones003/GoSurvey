...
- Pin: tag `0.13.4` (commit e3774bd4020fcfebb68150361db74b8b34d170fe)
- REQ-170 / ADR-041. Product licence is GPL-3.0-or-later because of this dependency.
- Upgraded from 0.13.3 (issue #366): 0.13.4 adds brute-force extraction of SAB ACIS
  data out of the `AcDsPrototype_1b` (ACDS) datastorage section for R2013+
  `3DSOLID`/`BODY`/`REGION` entities (`src/acds.spec`), populating `acis_data` on the
  entity the same way pre-2013 inline SAT/SAB data always has. This does NOT help
  every R2013+ solid — a solid whose entity carries no ACIS data at all (e.g. a
  parametric Civil3D/Plant3D catalog part, where the shape is generated by the host
  application's own catalog engine rather than stored portably) still comes back
  empty; only files that genuinely have SAB data stashed in the ACDS section benefit.
  Issue #301 (SAB parsing in `AcisSatParser`) is the follow-up needed to actually
  consume this newly-reachable binary data.
