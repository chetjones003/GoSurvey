...
- **REQ-341** (new, accepted 2026-09-10, D-2026-09-10-e) — the requirement this delivers.
- **ADR-058** (new) — the six decisions behind it. A new render capability needs an ADR and an
...
one criterion of one phase, and REQ-341 + ADR-058 (e) now carry the cost of closing it in writing.
...
**`headless.req341-section-clip` — 87 steps.** Every spelling and alias; every refusal with the
...
**The GUI check RAN, and it found a bug.** `--devshell-run req341-section-clip-viewport` builds a
...
which is ADR-058 (f)'s uncapped cut seen directly; the **complement** slab under FLIP; and the whole
...
- Requirements satisfied:  REQ-341 (new, accepted); GitHub #149 acceptance 6
...
- Assumptions:             none
- Architectural decisions: ADR-058, D-2026-09-10-e
- Dependencies:            none added
- Technical debt noted:    the uncapped cut and the ImGui-overlay entities are recorded as REQ-341
                           increments, each with the cost of closing it written down
- Build:                   reproducible, clean on Windows/MSVC
- Docs updated:            REQ-341 + its traceability row, ADR-058, D-2026-09-10-e, this task log
...
| 6 | **section clipping, live** | **this task, REQ-341 + ADR-058** |
...
rather than typed. Folded into this task rather than filed as a follow-up, because REQ-341 has not
...
`--devshell-run req341-section-clip-links` opens the prompt and **clicks each of the three links**,
...
being drawn, not the cut face — a solid whose cut is genuinely filled is REQ-341 increment 2, and
...
touches the routing layer — which is exactly why `req341-section-clip.txt` and the extended
