...
- Statement: **FILLET replaces a solid's sharp edge with a surface of constant radius `r`, traced by
  ...
  4b. **AMENDED 2026-09-08 (D-2026-09-08-c, ADR-046 amendment (l)): the check above is per-EDGE, and
...
- Status: accepted — **increments 1 and 2 delivered** (2026-09-05 / 2026-09-08). Increment 1: the kernel (TASK-210) and
  ...
  **Still refused, each by name and each its own increment:** a corner with only SOME of its edges
  ...
  REQ-329, accepted and delivered 2026-09-08 — it shares the edge selection and the refusal shape but none of the surface
  geometry.
- Revisions: 2026-09-05 — initial. Scope (straight convex edges between planar faces; a shared
  ...
  bullets above, and D-2026-09-05-c. 2026-09-08 — increment 2 (D-2026-09-08-a): edge chains and the
  ...
  AMENDED as item 4b (D-2026-09-08-c, ADR-046 amendment (l), TASK-223): the precondition was
  ...
  1290.97336. Found by a code-review pass on REQ-329, which had inherited the same check.



### REQ-329 — CHAMFER a solid's edge: replace a sharp edge with a flat bevel
...
- Statement: **CHAMFER replaces a solid's sharp edge with a flat bevel that cuts back the same
  ...
  13. **The second distance is deferred, by decision** (D-2026-09-08-b). AutoCAD's solid chamfer —
...
- **The gap recorded here on 2026-09-08 is CLOSED** by D-2026-09-08-c / ADR-046 amendment (l): item
  ...
  same change.
- Revisions: 2026-09-08 — initial. 2026-09-08 — item 4 AMENDED as item 4b (D-2026-09-08-c,
...
- Statement: this increment covers **Line-vs-Line TRIM only** — both the target being shortened and
  ...
  **Deferred, refused by name rather than silently run through the old flat math:** a skew
  ...
  pre-existing 2D implementation), remain open follow-on scope beyond issue #399's three stated
  increments. A true curved (tilted-arc) polyline segment intersection is likewise open follow-on
  scope. Plan view (world UCS, no orbit — pick ray is null) is completely unaffected for every entity
  type: `SubmitTrimViewportPick` falls through to the original, byte-identical 2D code path whenever
  no pick ray is supplied.
- Acceptance:
  ...
  - plan view / world UCS TRIM (no pick ray) is bit-for-bit unchanged for every entity type, including
    Polyline cutting edges and targets (regression guard).
- Owner-layer: Commands (`CadCommands.cpp`: `SegSegClosest3D`, `CutterCurvePlaneAndConic`,
  `FindNearestPolylineSegment3D`, `Try3DLineTrim`, `SubmitTrimViewportPick`'s `pickRay` parameter) /
  UI (`CadUi.cpp`'s `TrimPick` route, passing the click's already-computed `pickRayPtr`).
- Status: **accepted (2026-09-07)** — all three planned increments delivered: increment 1
  (Line-vs-Line), increment 2 (Circle/Arc/Ellipse cutting edges), increment 3 (Polyline as both
  target and cutting edge). Issue #399's own four draft acceptance criteria are now met for every
  entity combination it named, with the two 2D-parity gaps above (Circle/Arc/Ellipse as a TRIM
  target; true curved-polyline-segment intersection) named as explicit follow-on scope rather than
