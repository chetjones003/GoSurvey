...
- Acceptance:
  ...
  - **a standalone `.sat` file** (Civil 3D / AutoCAD `ACISOUT`) written in the real ACIS/ASM record
    schema imports through `BLOCKIMPORT` and the Import Block file dialog as a block definition named
    after the file (units from the ACIS header's millimetres-per-unit), its solid re-centred on the
    origin; `INSERT` then places that solid at the insertion point (a solids-only block leaves no
    reference). The real 4" weld-neck flange fixture (`samples/CJ_4in_WELD_NECK_FLANGE.sat`) —
    16 analytic faces including two planar faces pierced by an 8-bolt circle, and a `body` placement
    transform — imports as a valid `brep::Solid` (D-2026-09-10-c, ADR-051 addendum, GitHub issue
    #473).
...
- Revisions: 2026-09-05 — initial (ADR-051, GitHub issue #299).
  ...
  (block definition, solid re-centred on the origin); INSERT now materialises a block's B-rep solids
  at the insert point (translation / Z rotation / uniform scale), closing the pre-existing gap where
  a block-stored solid was never instanced.
...
| REQ-320 | IO/Domain/Commands | increment 1 delivered (GitHub issue #299, ADR-051, TASK-205) — SAT-only ACIS `3DSOLID` import of analytic primitives, refuse otherwise; `AcisSatParserTests`. **Amended 2026-09-10** (D-2026-09-10-b, ADR-051 addendum, GitHub issue #369, TASK-244): a payload-less `3DSOLID` co-occurring with `AECC_*` classes is a Civil 3D parts-catalog component with no portable geometry — named as `3DSOLID(Civil3D parts-catalog part, no portable geometry)` (conservative two-sign detection, `libredwgcad_detail::DwgHasCivil3dCatalogClasses`) and its block's 2D content is kept, not refused wholesale. `LibreDwgCadTests [issue369]`. **Amended 2026-09-10** (D-2026-09-10-c, ADR-051 addendum, GitHub issue #473, TASK-246): `AcisSatParser` now reads the real ACIS/ASM record schema — `NormalizeRealAcisSchema` rewrites real-format records onto the simplified layout, `Build` applies the `body` `transform` (rigid rotate + translate + uniform scale; reflection/shear refused), and a multi-hole planar face goes in as an ADR-052 `paramLoops` general trim loop (`BuildPlaneGeneralTrim`, the plane counterpart of `BuildConeGeneralTrim`). `.sat` is an importable file type in `BLOCKIMPORT` and the Import Block dialog — a block definition named after the file (units from the ACIS header `num_mm_units`), the solid re-centred on the origin (a `.sat` carries its absolute position from the source drawing, ~4999 units out for the fixture). **INSERT / EXPLODE now materialise a block's `CadBlockContent::solids`** into real drawing solids transformed by the insert frame (translation / Z rotation / uniform scale; a tilt or non-uniform scale on the reference is dropped with a logged note) — a solids-only block leaves no reference — closing the pre-existing gap where a block-stored solid was never instanced. Fixture `samples/CJ_4in_WELD_NECK_FLANGE.sat` (real Civil 3D `ACISOUT` flange, 16 faces). `AcisSatParserTests [issue473]`, `CadBlockImportTests [issue473]`, `headless.issue473-sat-blockimport`. A *linked* block-reference solid that re-derives from the definition is still future. | accepted |
