...
- Acceptance:
  ...
  - **Each validity fault is reported APART from the others** (amended 2026-09-09, D-2026-09-09-j,
    for GitHub #149 acceptance 7): an unclosed shell, a non-manifold one, an inconsistently oriented
    one and a degenerate edge or face each carry their own reason, and no two of them collapse onto
    one message. *This was already true* — the amendment pins it, because a probe using careless
    fixtures reported otherwise and nearly bought a "fix" for a defect that did not exist. Ring
    closure is checked **before** the edge-use tally, so a fixture that disturbs a loop's ring never
    reaches the tally and reports a broken loop whatever fault it was built to test. A test for one
    of these faults must therefore assert the **specific** reason, never merely that validation
    failed.
...
| REQ-313 (validation reported) | Domain/Commands | accepted, delivered (GitHub issue #149 acceptance 7, D-2026-09-09-j, TASK-239). `SOLIDCHECK` — validity per solid, plus the separate geometric question of self-intersection, over the selection or the whole drawing. Read-only. **No kernel change was needed and none was made**: `brep::Validate` already reported an unclosed shell, a non-manifold one, an inconsistently oriented one and a degenerate edge under four distinct `Problem` values with four distinct messages. The phase gameplan had budgeted work to "split two colliding values" on the strength of a probe that saw two faults report the same text — **the probe's FIXTURES were at fault**: `Validate` checks ring closure before it tallies edge uses, so a fixture that disturbs a loop never reaches the tally, and both of its broken boxes reported `LoopNotClosed` whatever they were built to test. Re-probed with fixtures that reverse a loop's ORDER as well as its flags, and that duplicate a whole FACE rather than a loop, all four come back apart. `BrepTests [req149]` now pins that as an assertion — five cases, each asserting the SPECIFIC `Problem` and one asserting all four are pairwise distinct — and `headless.req313-solidcheck` covers the command, including the self-passing torus that `Validate` correctly calls Ok while its measurements are withheld | accepted |
