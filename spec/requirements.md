...
- Status: accepted — **increment 1's KERNEL implemented 2026-09-05**; the `FILLET` command is the
  next slice (D-2026-09-05-c, GitHub issue #148 Phase 5 acceptance 5). `brep::FilletEdge` rounds one
  straight convex edge between two planar faces and `brep::FilletEdges` applies several that share
  no vertex as one operation, each refusal by name and pre-checked before anything is built.
  Increment 2 is the spherical corner patch, which is what closes issue #148's acceptance 5;
  increment 3 is the concave edge; increment 4 is an OBLIQUE end face, whose boundary is an ellipse
  rather than a circle; a circular rim between a plane and a cylinder (a torus fillet) is its own
  requirement and is where REQ-321's general trim loops first become necessary. CHAMFER is a
