...
- Scope boundaries, stated rather than left silent:
  ...
  - **The document-facing half is a separate increment** — the `CadSolid` entity, the seven commands
    with exact typed dimensions, `.gs` persistence, rendering under REQ-064's visual styles, the
    cached tessellation and its REQ-100 budget, face and edge snapping, and the DXF/DWG exclusion
    message of ADR-045 (i). Issue #146's remaining acceptance bullets are met there. The split is
    deliberate: this increment changes no existing source file, so it cannot regress anything, and a
    create-command on `beta` before persistence exists would let a user build a solid and lose it on
    save, which is the silent failure REQ-201 forbids.
- Owner-layer: Domain (`src/util/brep.hpp`, `src/util/brep.cpp`)
- Status: accepted (2026-09-01) — see D-2026-09-01-a and ADR-045.
- Revisions: 2026-09-01 — proposed and accepted (D-2026-09-01-a, TASK-164). Phase 3 of GitHub #120,
