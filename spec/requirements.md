...
- Status: accepted (2026-08-25)
- Revisions: 2026-08-25 — accepted (D-2026-08-25-m); issue #81.
  2026-08-25 — amended (D-2026-08-25-n). Two things, one review and one collision. The
...
| REQ-119 | UI/Commands | planned — `CommandLineTests` (the prompt→variants rule as a pure function: inline `[A]zimuth`/`[2P]`, grouped `[Yes/No]` and `[DElta/Percent/Total/DYnamic]`, FILLET/CHAMFER's `[Radius/Trim]`/`[Distance/Angle/Trim]`/`[Trim/No trim]`, mixed-case shortcut extraction incl. `No trim`→`N`, unclosed bracket, empty group, and a round-trip so parsing loses no text) + `headless.regression-119-variant-token-accepted` (every extracted token is ACCEPTED by the command showing that prompt — LINE a/2p, MIRROR y erases / n keeps by entity count, LENGTHEN de/p/t/dy each from a fresh `WaitSelectOrMode`, plus the old parser's token kept as a live rejection; note click≡type is true by construction and is deliberately NOT what this asserts) + manual (links render and hover in BOTH the floating bar and the classic dock; a wrapping dock prompt keeps its links on the correct line with no horizontal overflow; **no log line is clickable**) | accepted |
