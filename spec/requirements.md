...
- Acceptance:
  ...
  - **a standalone `.sat` file** (Civil 3D / AutoCAD `ACISOUT`) written in the real ACIS/ASM record
    schema imports through `BLOCKIMPORT` and the Import Block file dialog: it becomes a block
    definition named after the file (units from the ACIS header's millimetres-per-unit) and its
    solid is also placed in the drawing. The real 4" weld-neck flange fixture
    (`samples/CJ_4in_WELD_NECK_FLANGE.sat`) — 16 analytic faces including two planar faces pierced
    by an 8-bolt circle, and a `body` placement transform — imports as a valid `brep::Solid`
    (D-2026-09-10-c, ADR-051 addendum, GitHub issue #473).
...
- Revisions: 2026-09-05 — initial (ADR-051, GitHub issue #299).
  ...
  (block definition + direct placement). Placing a *block-stored* solid stays a tracked gap.
