...
- Acceptance, increment 2:
  - Each of the seven primitives is created by **one typed line** — a base point in the active UCS
    ...
    argument. **There is no interactive pick-and-drag placement**: that needs a 3D draft preview and
    is #120's Phase 5 work, and the usage text says so rather than opening a prompt that never comes.
  - Every bad input is **refused by name and creates nothing** (REQ-201): a non-positive dimension, a
    cone or pyramid top radius at or above the base, a torus whose tube would pass through its own
    axis, a side count outside 3..64, a **fractional** side count (refused, never truncated to a
  ...
  - The tessellation is **cached** on `(solid, chord tolerance)` and regenerated on nothing else, so
    it is not rebuilt per frame (#120). `BENCH SOLID` measures profile (d) of REQ-100; the instrument
    is delivered and the number is not yet taken — see REQ-100's status.
...
- Status: accepted — **profiles (a), (b) and (c) MET, measured 2026-08-15** (TASK-052, TASK-053).
  **Profile (d), solids, is NOT YET MEASURED** — the `BENCH SOLID` instrument exists (TASK-167) but
  ...
  scene is a different shape of work, which is the whole reason it is a separate profile. Every
