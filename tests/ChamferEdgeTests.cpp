// REQ-329 (D-2026-09-08-b, ADR-046 amendment (k), GitHub issue #148 acceptance 5) — CHAMFER:
...
//
//   * the WEDGE, which proves REQ-329 item 2: the chamfer's setback is exactly `d` at a dihedral of
...
TEST_CASE("Chamfer: a box edge, against the closed form", "[chamfer][req329]") {
  ...
  // REQ-329's acceptance, term by term so a failure names its own cause:
...
TEST_CASE("Chamfer: the setback does NOT follow the dihedral", "[chamfer][req329]") {
  // The mirror image of REQ-323's wedge case, and REQ-329 item 2's proof. `MakeWedge` is full height
...
TEST_CASE("Chamfer: what it refuses, each by name", "[chamfer][req329]") {
  brep::Solid cyl;
  Problem why{};
...
TEST_CASE("Chamfer: two coplanar faces meeting an edge are refused", "[chamfer][req329]") {
...
TEST_CASE("Chamfer: a concave edge is refused, not silently built", "[chamfer][req329]") {
  // REQ-329 item 6, inheriting REQ-323 item 6's reasoning unchanged: a bevel on a crease ADDS
...
TEST_CASE("Chamfer: an oblique end face is refused, and the FIRST answer was wrong",
          "[chamfer][req329]") {
  // REQ-329 item 10 / ADR-046 amendment (k)(5). This case exists because the requirement was
...
TEST_CASE("Chamfer: a chamfered solid is still a solid the next operation accepts",
          "[chamfer][req329]") {
  // REQ-329's `PRESSPULL` acceptance bullet. A chamfer that left a shape later operations refused
...
TEST_CASE("Chamfer: every refusal has its own message, not the generic solid-invalid one",
          "[chamfer][req329]") {
  // REQ-329 item 5: "refused by name". `ProblemText` is the one place that name reaches a user, so a
...
// --- REQ-329 increment 2: edge chains, and the corner that is a POINT ------------------------------
//
...
TEST_CASE("Chamfer: three edges at one corner meet at a POINT, not a patch", "[chamfer][req329]") {
...
TEST_CASE("Chamfer: a corner with only some of its edges selected is refused",
          "[chamfer][req329]") {
  // REQ-329 item 8. The bevel would have to run off onto an edge staying sharp — a setback blend,
...
TEST_CASE("Chamfer: a corner whose faces are not square to each other is refused",
          "[chamfer][req329]") {
  // REQ-329 item 9 / amendment (k)(4) — refused, but NOT for the fillet's reason. There is no patch
